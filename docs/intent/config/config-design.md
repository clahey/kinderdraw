...
Raw Settings are what's actually persisted, behind a shared Kotlin interface (see the HLD's cross-platform data-layer boundary decision) that each platform provides its own implementation of:
...
| Age/mode resolution | A single platform-agnostic resolver, built on the shared Raw Settings interface, computing resolved per-feature values | Each platform implements its own age-to-bundle mapping independently; consumers (e.g. User Experience) read Raw Settings directly and compute resolution themselves | The age-to-bundle mapping is business logic, not storage or UI — implementing and sharing it once removes any chance of Android and Linux disagreeing on what a given age enables, and keeps consumers from needing to know about raw mode/age at all. |
| Resolver's relationship to Raw Settings | A separate class that takes a Raw Settings implementation as a constructor dependency | Fold resolution logic into the Raw Settings interface itself (e.g. default methods) | Keeps the interface a pure per-platform contract with no shared logic riding along inside it; the resolver stays fully platform-agnostic and testable against any Raw Settings implementation, including a fake for tests, without being part of that implementation's own inheritance chain. |
...
- Root HLD: `docs/high-level-design.md` — System Design (shared local data store), Approach (age-adaptive canvas UX, first-launch adult setup), Key Design Decisions ("one shared local data store, not two"; cross-platform data-layer boundary as a shared Kotlin interface).
