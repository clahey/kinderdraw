...
- **No advertising**, which costs nothing today: the app carries none on any screen, and the HLD forecloses it permanently on the kid canvas while leaving the companion screen open.
...
- Foreground: one stroke, drawn as two segments so the color changes partway — brand light blue for the first half, brand green for the second. The segments are drawn blue-first so that **green passes over blue where the line crosses itself**, which is what makes the crossing read as one line going over its own path. That is a property of draw order rather than of color choice, so it has to survive any re-authoring; collapsing the two segments into a single path loses it.
- Three things hold the two segments together as one line, and the drawables are unforgiving about all three. The color changes *underneath* the crossing rather than at a point in the open, so the seam is never seen. Because it is never seen, the green needs no cap to conceal it and is flat at both ends where the rest of the line is round — a round or square cap there reaches past the blue it is laid over and shows as a rim of light blue, which no cap shape fixes. And the control points either side of the seam are colinear through it, without which the halves read as two curves that meet rather than one line changing color. Moving where the color switches means re-establishing all three: a new seam under the crossing, colinear handles derived for it, and the caps re-checked.
...
| Promises the copy makes about the future | One: that an account will never be *required* to draw. Everything else — no ads, no purchases, no subscriptions, no network — is stated in the present tense only | Promise the absence of ads permanently across the whole app; state each of the others as permanent too | A listing claim can't be quietly retracted, and it is read both by people deciding whether to trust the app with a child and by Play alongside the Data safety declaration — so the present tense is what the copy can always stand behind, whatever the app becomes. The ads promise is left unmade for a second reason as well: the HLD forecloses ads only on the kid canvas and leaves the companion screen open, so a blanket permanent promise would outrun the intent it claims to rest on. The account exception earns its place by being narrow — an optional account stays imaginable, while a *required* one is what a parent is actually checking for, and the HLD's Non-Goals rule it out |
...
| Monetization | Free, no ads on any screen, no paywall, open source under MIT | Ads on the companion screen; a paid tier; in-app purchases | The free-and-open-source tenet settles the licence and the price. Ads are a separate question the tenet does not answer, and the HLD closes it only for the kid canvas — the companion screen stays open, so shipping without ads there is this release's choice rather than a permanent commitment. It is the choice because the app is child-directed for the whole listing: ads on any screen would pull it into the Families ad requirements, which is real work for revenue this release has no need of |
