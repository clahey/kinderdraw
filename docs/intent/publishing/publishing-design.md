...
4. **App icon** — ✅ in place as `androidApp/src/main/res/drawable/ic_launcher_{background,foreground,monochrome}.xml` with `mipmap-anydpi-v26` entries. `minSdk 30` clears the API 26 floor for adaptive icons, so no PNG density buckets are needed. The 512×512 store icon is separate and still pending — see Store Listing Assets.
...
6. **Release build type** — ✅ `release` in `androidApp/build.gradle.kts`, deliberately with no `signingConfig` (see Decisions).
7. **Privacy policy** — **mandatory**, not optional, because the app is child-directed. Written as `PRIVACY.md` at the repo root, to be hosted at its GitHub-rendered URL so no infrastructure is needed. Its claims are grounded in the merged release manifest, which declares no internet permission and no advertising ID. Its contact address is still a placeholder.
8. **Store listing** — title, short description, and full description are written (`docs/store-listing/listing-copy.md`); screenshots and the 1024×500 feature graphic are not.
...
| Store category | Art & Design | Education | The app is a drawing tool, not a teaching one, and it makes no educational or developmental claim — the listing copy deliberately avoids one, since there is no evidence behind it and it would invite scrutiny the app has no reason to attract. Education would also place it against apps promising outcomes this one doesn't. Reversible in the Console at any time, and category mainly drives browse rather than search |
...
- `docs/brand.md` — the brand palette; canonical source for every hex named here (arriving with the interaction-lock branch — see Active #5)
