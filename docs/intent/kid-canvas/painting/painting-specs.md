...
- [x] **CANVAS-PAINT-024**: When an input event carries a pointer that is neither pressed nor newly lifted — a hovering pointer — the system shall neither consume that change nor record it against any stroke, leaving the drawing and the interaction untouched. Verified by inspection rather than by test — the test harness injects touch, and a touch pointer is always pressed, so no test can construct a hovering one.
...
- [x] **CANVAS-PAINT-025**: When the snapshot operation is called, the system shall render the current drawing to a raster image — the same off-screen rendering, at the same pixel size, that the save operation rasterizes with — and return that image to the caller without writing it to Image Storage or altering the drawing in any way.
