...
- **Color selection** — Widgets owns the visual mechanism (e.g. which swatch shows as active); User Experience owns Active Stroke Settings, the resolved brush and background source Painting reads from (see the Painting LLD) — color is a property of the brush instance Active Stroke Settings resolves, not a separate value, so writing the toddler's swatch selection into Active Stroke Settings means constructing a freshly colored brush instance for the next resolved-brush query. The next stroke Painting starts is simply in the new color, with no separate acknowledgment step required. Brush *type* has no selection input yet — Active Stroke Settings always resolves an instance of Painting's single default brush type, just constructed with the current color (see the Painting LLD's Brushes). The drawing surface's background is resolved the same way but has no selection input at all yet — no Widgets control writes into it, so Active Stroke Settings' implementation returns a fixed default (see the Painting LLD's Active Stroke Settings section for when Painting queries it).
...
| Signal a live gesture's blocking-layer needs | Widgets: each control's own `onPressedChange` callback. Painting: User Experience already drives Painting's `onPointerDown`/`onPointerUp` from its own pointer-input handling, so it observes stroke start/end directly with no new signal needed | A dedicated "gesture live" event/state exposed by each component | Widgets' controls run their own internal pointer-tracking loop User Experience doesn't otherwise see into, so they need an explicit callback; Painting's input methods are already called by User Experience's own code, so the same call sites that drive Painting already are the signal — adding a second, redundant one would just be more surface for the two to drift out of sync. |
...
| Ownership of Active Stroke Settings (Painting's resolved brush and background source) | User Experience implements and owns it, writing the toddler's swatch selection into it as the color to construct the next resolved brush instance with (brush type has no selection input yet, so it always resolves an instance of Painting's single default type); the resolved background has no write path yet, so it's a fixed default | Painting owns an active brush directly as its own mutable state, written via a setter call from User Experience | Matches the Painting LLD's decision to keep Painting's stroke rendering ignorant of how a resolved brush gets decided. User Experience already owns Widgets' color-selection wiring, so it's the natural place to decide — and later evolve — what the resolved brush and background are. |
...
4. Whether a resolved brush (including its color) can ever change automatically between strokes, without a new toddler tap, isn't decided — Active Stroke Settings' interface shape allows for it, but no such behavior exists today.
5. How Active Stroke Settings' implementation constructs a correctly colored brush instance once Painting supports more than one brush type is deferred to the Painting LLD's Open Questions (#9) — today's single-brush-type implementation just constructs that one type directly with the current color.
6. Ownership of the HLD's first-launch adult setup dialog (age slider, device-lock button, link into the Companion App, "don't show again" toggle) isn't assigned to any Kid Canvas component yet. User Experience, as the kid canvas's composition root, is the likely owner, but this LLD doesn't cover it — today's screen composition and lifecycle behavior are defined without it.
7. What resolved-feature values User Experience reads before Config's UX configuration has ever been written (first run, before any dialog or Companion App write) is Config's own default-resolution question, not decided here — see the Config LLD.
8. Whether Painting becomes a hoisted-state Compose pattern (a `PaintingState` object plus a thin composable wrapper owning its own pointer input) is Painting's own open question (see its LLD's Open Questions #15), not decided here — but if it happens, Painting would need to report stroke start/end to User Experience itself, replacing today's "no new signal needed" shortcut in the Input Arbitration Decisions & Alternatives, since User Experience would no longer be the one directly driving Painting's pointer input.
