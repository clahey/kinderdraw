...
**Blocking.** For as long as one component holds a live gesture — a Widgets control between its `onPressedChange(true)` and matching `onPressedChange(false)`, or Painting between its `onStrokeActiveChange(true)` and matching `onStrokeActiveChange(false)` (see the Painting LLD's Reporting Stroke State) — User Experience composes one transparent, pointer-consuming layer over the entire screen, for that duration only. A new pointer-down landing on that layer is consumed there and never reaches Widgets or Painting underneath, so it can't claim a control, start a stroke, or re-trigger the control already holding the gesture. This doesn't disrupt the holding component's own gesture: Compose fixes a pointer's target at its own down event, so the pointer already claimed before the layer appeared keeps going straight to whichever component claimed it, layer or no layer. Because of that same fixed-target behavior, a pointer swallowed by the blocking layer stays swallowed for the rest of its own gesture even if the live gesture ends while it's still held down — only a fresh pointer-down after that is eligible to start the next gesture.

This also means a Widgets action, including New Picture, can only start once no stroke is active on Painting — there's no case where a lifecycle action fires mid-stroke. New Picture then holds the live-gesture slot for its own entire sequence (see Lifecycle Behavior), not just the initial claim that starts it: the blocking layer stays up from the moment New Picture's pointer is claimed (`onPressedChange(true)` — see the Widgets LLD's Reporting Press State) through `clear()` completing, so a touch during that window is blocked outright rather than accepted as a new stroke the sequence's `clear()` step would immediately erase. That hold starts at claim regardless of whether the press goes on to actually activate New Picture — see Lifecycle Behavior for where the sequence itself begins.
...
Triggered by New Picture's `onActivate` (see the Widgets LLD's Reporting Press State) — reachable, per Input Arbitration above, only when no stroke is active on Painting. The action runs a fixed sequence:
...
The blocking layer that covers this sequence (see Input Arbitration) is already up before the sequence even starts — it goes up the moment New Picture's pointer is claimed, not at `onActivate` — and stays up through step 3 completing, including across the asynchronous `save()` call in step 2, rather than being released and re-acquired at the press's own release. No new stroke or control-press can start anywhere in that window, from the initial claim through the sequence's end.
...
| Input arbitration between Widgets and Painting | Two mechanisms: ordinary Compose z-order dispatch decides which component *claims* a fresh touch (Widgets on top, Painting beneath); a User-Experience-composed transparent blocking layer over the whole screen keeps every other pointer out for the live gesture's duration | A single hand-rolled arbiter that inspects every raw pointer itself and manually routes it to Widgets or Painting; per-touch heuristics to classify incidental vs. intentional touches; redirect an imprecise touch to the nearest control it probably meant to hit; a layer covering only the non-holding component, leaving the holding component newly reachable by a second, different pointer | Claiming is already exactly what Compose's own hit-testing does for free when Widgets sits above Painting — reimplementing it by hand would just be a slower, bug-prone copy of the toolkit's own dispatch. A whole-screen blocking layer is the smallest addition on top of that default behavior: it costs nothing extra over covering only the non-holding side, since Compose fixes a pointer's target at its own down event and so never disrupts the holding component's already-claimed pointer, and it additionally protects the holding component itself from a second concurrent pointer without either component needing to know the other exists. |
...
4. ✅ The blocking layer covers the entire screen unconditionally whenever any gesture is live, rather than only the non-holding component — see Input Arbitration's Blocking section.
5. ✅ `KidCanvasScreen` composes Widgets (New Picture) and the blocking-layer overlay alongside Painting — see Screen Composition, Input Arbitration, and Lifecycle Behavior's New Picture section.
