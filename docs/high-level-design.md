...
- **First-launch adult setup, on the kid canvas itself** — the kid canvas shows one dialog at launch, freely dismissible with no required action, that gives an adult everything needed to hand the device to a toddler without first discovering the companion app: an age slider that seeds the same age-adaptive configuration the companion app controls, a button to lock the device to the app where the platform supports it (falling back to on-screen instructions where it doesn't), a link into the companion app, and a "don't show again" toggle. The dialog reappears on every launch until that toggle is set. This is the one dialog ever placed in front of the kid canvas, and it's addressed to the adult setting the device up, never to the toddler or in response to something the toddler did — see Goals and Tenets.
...
- **Parents/caregivers** — secondary users, via the companion screen. Need to browse and manage what their child has drawn.

## Goals

- A toddler can draw, change color, and start a new picture without needing an adult's help after a brief demonstration.
- No blocking confirmation dialogs are ever shown to the toddler on the kid canvas, and none is ever shown in response to something the toddler did. The sole exception is the first-launch adult setup dialog, which is addressed to the adult setting the device up, not the toddler.
- An adult can, on first launch of the kid canvas, set an initial age and lock the device to the app, without needing to already know the companion app exists.
- The app is free on every platform, and the kid canvas never shows an ad.
...
- Advertising on the kid canvas, on any platform. Whether the companion screen could ever carry advertising is left open — see Key Design Decisions.
...
- **Free and open-source.** kinderdraw costs nothing to use, and its source is public under an open licence.
...
- **Cross-platform data-layer boundaries as a shared Kotlin interface, one implementation per platform.** Where shared code depends on a data-layer boundary that differs per platform (Config, Image Storage), the boundary is declared once as an interface in shared Kotlin code; each platform provides its own concrete implementation, instantiated by that platform's own app shell, and remains free to also extend whatever platform-specific persistence helper it needs since Kotlin's single-inheritance limit applies to classes, not interfaces. Shared consumers (the canvas, including Painting) depend only on the interface, never on a platform's concrete implementation directly. Logic that's genuinely shared and doesn't vary per platform — such as Config's age-to-feature resolver — is ordinary shared code, built as its own class that takes the interface as a constructor dependency rather than being folded into the interface itself.
- **No advertising on the kid canvas; the companion screen left open rather than promised.** The commitment that matters is that the toddler surface never carries an ad — a toddler cannot tell an ad from the app, and a parent handing over the device is trusting exactly that. Extending the ban to the companion screen would over-commit: that screen is an ordinary adult surface, and nothing in the product thesis requires foreclosing it. Being free and open-source settles none of this either way, since open-source software is routinely ad-supported. But the companion case is left *open*, not planned, because it isn't free: the companion screen ships in the same APK and the same Play listing as the kid canvas, and that listing declares a child target-audience age band, so the whole app is child-directed under Play's Families policy. Ads on any screen would pull the entire app into the Families ad requirements — certified ad networks only, format restrictions, an advertising-ID declaration, and a Data safety change — or force the companion screen into a separate listing, which cuts against the shared-APK, shared-store design.
- **Android minSdk 30, targetSdk/compileSdk 36.** Android 11 (API 30) is where scoped storage's ownership model stabilized: an app can freely modify or delete MediaStore entries it created itself, with no OS consent dialog, and `MediaStore.createWriteRequest()`/`createDeleteRequest()` became available for the (for this app, unused) case of touching media it doesn't own. Floors below 30 either predate scoped storage entirely (API < 29, a different legacy filesystem-access model) or landed on API 29, where that same-app-owned-item exemption behaved inconsistently across devices — either era would need its own fallback code path in Image Storage's Android backend for no real gain, since minSdk 30 already covers the large majority of active Android devices. targetSdk/compileSdk 36 matches Google Play's current publishing requirement.
- **First-launch adult setup as an in-app dialog on the kid canvas, over relying on the companion app alone.** An adult handed only the kid canvas (its own launcher icon or the device's default view) has no reason to know a separate companion entry point exists, so age setup and app-locking need an on-ramp reachable from the kid canvas itself. Alternatives considered: requiring the adult to find the companion app unaided (fails the goal of not needing prior knowledge); OS-level guided access/parental-control instructions only, with no in-app dialog (leaves age setup with no entry point at all); a persistent settings icon on the kid canvas instead of a first-launch dialog (stays reachable by the toddler afterward, and doesn't naturally prompt the one-time actions this dialog exists for). The dialog is freely dismissible with no required action, consistent with the reversible-actions-default-to-forgiving tenet applied to the adult's own experience of it. Whether device-locking can be invoked programmatically, and how it degrades where it can't, is platform-dependent and deferred to the kid canvas component's own LLD.
...
- No ads or purchase prompts ever appear on the kid canvas, and no blocking confirmation dialog is ever shown to the toddler or in response to something the toddler did — only the adult-directed first-launch setup dialog is exempt.
