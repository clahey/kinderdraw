...
graph TD
    App[kinderdraw APK]
    Canvas[Toddler Canvas<br/>custom pointer input]
...
One Android app module. Two separate entry points — the toddler canvas and the companion app are reached independently (not one gated behind the other) — sharing one local on-device data store in-process. That store holds saved drawings, app settings, and the current UX configuration (age-slider value or custom per-feature toggle state) that the companion app writes and the canvas reads to determine which features are active.
...
- No ads, purchase prompts, or blocking confirmation dialogs ever appear on the toddler canvas.
